...
async function newSession(flash){
  ...
  const requestedModel=(window._managedModelLocked&&window._managedModelValue)
    ? window._managedModelValue
    : $('modelSelect').value;
  const data=await api('/api/session/new',{method:'POST',body:JSON.stringify({model:requestedModel,workspace:inheritWs})});
